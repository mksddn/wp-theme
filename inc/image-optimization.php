...
function wp_theme_image_optimization_init(): void {
    ...
    // Remove intermediate image sizes based on selected sizes
    // Auto-enable if any sizes are selected for removal
    if (!empty($settings['image_opt_remove_sizes_list'])) {
        ...
        // Also disable scaled images if removing sizes
...
function wp_theme_optimize_image_upload(array $file): array {
    ...
    $max_size = $settings['image_opt_max_file_size'] * 1024 * 1024; // Convert MB to bytes
